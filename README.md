...
The latest ten usable items appear on four consecutive slides (3, 3, 3, and 1 item). Shorter feeds use only the necessary slides. Valid publication dates determine newest-first order when every item has one; otherwise the publisher’s feed order is retained. Headlines use the embedded monospace font at 56 points, spanning the full content width with thin horizontal separators and no side QR codes. Each headline has two lines; longer titles end with an ellipsis. A single enlarged QR code (up to 560 pixels, with a white quiet zone and crisp integer-sized modules) is centered in the bottom area and links to that slide’s first headline (items 1, 4, 7, and 10 respectively). Headlines appear in feed order. Items without usable HTTP(S) links are skipped; a URL too dense for the bottom QR omits the code without hiding the headlines. The QR has no caption. News is rendered from the latest successful poll without an image cache.

Preview with `/preview?screen=rss`, which rotates through the news pages, or select a numbered News preview link. Use `screen: rss` to rotate only the news pages on the device. The direct preview shows an empty-state message when news is unavailable. Restart after changing configuration.
...
Only the list of configured newspaper names/IDs is kept and refreshed every 15 minutes. Until the first catalog response, fortune, calendar, quarter progress, and weather continue. Removed papers disappear on the next successful catalog refresh. If a cover fetch fails (including HTTP 502), the image request serves a fortune slide with the device's battery footer so the slideshow continues normally. The error is logged, and the next cover request fetches live again; neither an old edition nor the fallback is cached under the cover URL. A catalog outage preserves the newspaper list, not any cover images.

Each cover gets its own full-screen grayscale slide and the shared battery footer. Cropping is detected anew from each edition's image dimensions. **El Nuevo Día (ENDI) shows the full portrait page, or the full-height right half of landscape or square spreads.** Other newspapers keep the existing newsstand zoom: **portrait single pages show the upper half**, preserving the full width; **landscape or square spreads show the upper-right quadrant**. The selected area is enlarged proportionally without stretching. This dimension-based heuristic assumes wide sources contain two pages side by side; unusually shaped or padded source images may be misclassified. Source resolution limits readability; the server cannot add detail missing from a small source image. The current El Nuevo Día image is 468 × 492 pixels. Network timeouts, response-size limits, and an image-pixel limit bound failures. Cover URLs and redirects must stay on the configured frontpages origin.
